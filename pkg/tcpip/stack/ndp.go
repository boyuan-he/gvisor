...
func (ndp *ndpState) rememberDefaultRouter(ip tcpip.Address, rl time.Duration) {
	...
	state := defaultRouterState{
		invalidationTimer: ndp.nic.stack.clock.AfterFunc(&ndp.nic.mu, rl, func() {
			ndp.invalidateDefaultRouter(ip)
		}),
	}
...
func (ndp *ndpState) generateTempSLAACAddr(prefix tcpip.Subnet, prefixState *slaacPrefixState, resetGenAttempts bool) bool {
	...
	state := tempSLAACAddrState{
		deprecationTimer: ndp.nic.stack.clock.AfterFunc(&ndp.nic.mu, pl, func() {
		...
		invalidationTimer: ndp.nic.stack.clock.AfterFunc(&ndp.nic.mu, vl, func() {
			prefixState, ok := ndp.slaacPrefixes[prefix]
			if !ok {
		...
		regenTimer: ndp.nic.stack.clock.AfterFunc(&ndp.nic.mu, pl-ndp.configs.RegenAdvanceDuration, func() {
			prefixState, ok := ndp.slaacPrefixes[prefix]
			if !ok {
