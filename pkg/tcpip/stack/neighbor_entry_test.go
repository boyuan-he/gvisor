...
// fakeTimer allows manipulation of work scheduling.
...
func (t *fakeTimer) StopLocked() {
	t.clock.mu.Lock()
	defer t.clock.mu.Unlock()
	for i, item := range t.clock.work {
		if item.id == t.id {
			t.clock.work = append(t.clock.work[:i], t.clock.work[i+1:]...)
			return
		}
	}
}
...
func (t *fakeTimer) Reset(d time.Duration) {
	...
	t.clock.work = append(t.clock.work, workItem{
		id:        t.id,
		locker:    t.locker,
		executeAt: t.clock.now.Add(d),
		fn:        t.fn,
	})

	sort.Slice(t.clock.work, func(i, j int) bool {
		return t.clock.work[i].executeAt.Before(t.clock.work[j].executeAt)
	})
}
...
func (c *fakeClock) NewTimer(l sync.Locker, f func()) tcpip.Timer {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *fakeClock) AfterFunc(l sync.Locker, d time.Duration, f func()) tcpip.Timer {
	timer := c.NewTimer(l, f)
