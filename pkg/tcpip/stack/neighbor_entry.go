...
func (e *neighborEntry) setStateLocked(next NeighborState) {
	...
	case Incomplete:
		...
		sendMulticastProbe = func() {
			...
			retryCounter++
			e.cancel = e.nic.stack.clock.AfterFunc(&e.mu, config.RetransmitTimer, sendMulticastProbe).StopLocked
		}

		sendMulticastProbe()

	case Reachable:
		e.cancel = e.nic.stack.clock.AfterFunc(&e.mu, e.nudState.ReachableTime(), func() {
			e.dispatchChangeEventLocked(Stale)
			e.setStateLocked(Stale)
		}).StopLocked

	case Delay:
		e.cancel = e.nic.stack.clock.AfterFunc(&e.mu, config.DelayFirstProbeTime, func() {
	...
	case Probe:
		...
		sendUnicastProbe = func() {
			...
			e.cancel = e.nic.stack.clock.AfterFunc(&e.mu, config.RetransmitTimer, sendUnicastProbe).StopLocked
		}
	...
	case Failed:
		...
		e.cancel = e.nic.stack.clock.AfterFunc(&e.mu, config.UnreachableTime, func() {
